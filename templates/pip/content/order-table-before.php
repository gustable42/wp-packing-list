					...
					if ( 'pick-list' === $type ) :
						...
						if ( ! empty( $document->order_ids ) && 'category' === $document->group_items_by() ) :
							...
							foreach ( $order_ids as $order_id ) {
								...
								$orderSchedule = $wc_order->get_meta(_order_schedule_meta);
+								$orderSchedule = gmdate( 'd/m/Y H:i', $orderSchedule );
+
+								$edit_order_links .= '<a href="' . esc_url( get_edit_post_link( $order_id ) ). '" target="_blank">#' . $wc_order->get_order_number() . '</a>' . ' (Retirada em: '. $orderSchedule .') '. ' <br>';
							}

							/* translators: Placeholder: %s - order edit links */
														printf( '<p>' . __( 'Orders:<br> %s', 'woocommerce-pip' ) . '</p>', rtrim( $edit_order_links, ', ' ) );
